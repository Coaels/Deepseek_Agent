...
print(f"SkillManager 是否存在: {'SkillManager' in globals()}")
def is_admin():
    try:
        ...
        for filename in sorted(os.listdir(self.skills_dir)):
            ...
            try:
                ...
                if skill:
                    ...
                    print(f"✅ Skill 加载成功: {skill_name}")
            except Exception as e:
                print(f"❌ Skill 加载失败 {skill_name}: {e}")
...
def make_tool_summary(tool_calls_results):
    ...
    for tc_name, tc_args, result in tool_calls_results:
        ...
        if info.get('saved_file'):
            summaries.append(f"已保存文件: {info['saved_file']}")
        elif info.get('read_file'):
            summaries.append(f"已读取文件: {info['read_file']}")
        elif info.get('target_file'):
            summaries.append(f"操作目标: {info['target_file']}")
        elif info.get('command'):
            summaries.append(f"执行命令: {info['command'][:50]}")
        elif info.get('query'):
            summaries.append(f"搜索: {info['query']}")
    if summaries:
    ...
    for turn in range(max_turns):
        ...
        for idx in sorted(tool_calls.keys()):
            ...
            for st in skill_data['tools']:
                fn = st.get('function', {})
                tool_display[fn.get('name', '')] = f"🧩 {fn.get('name', '')}"

            yield {"type": "text", "content": f"🔧 工具: {tool_display.get(tool_name, tool_name)}\n"}

            if tool_name == "execute_command":
                yield {"type": "text", "content": f"▶ 命令: {tool_args.get('command', '')}\n"}
            elif tool_name in ["read_file", "write_file"]:
                yield {"type": "text", "content": f"📄 文件: {tool_args.get('filepath', '')}\n"}
            elif tool_name == "list_files":
                yield {"type": "text", "content": f"📂 目录: {tool_args.get('path', '.')}\n"}
            else:
                args_str = json.dumps(tool_args, ensure_ascii=False, indent=2)
                yield {"type": "text", "content": f"📦 参数: {args_str}\n"}

            yield {"type": "text", "content": f"{'─'*50}\n"}
            yield {"type": "heartbeat"}
            ...
            if tool_name == "execute_command":
                ...
                if tool_name == "search_web":
                    print(f"🔍 AI 搜索: {tool_args.get('query', '')}")
                elif tool_name == "search_images":
                    print(f"🖼️ AI 搜图: {tool_args.get('query', '')}")
...
def _format_tool_output(tool_name, result):
    if tool_name == "search_web" and "results" in result:
        ...
        lines = [f"🔍 [{engine.upper()}] 找到 {result.get('results_count', 0)} 条结果:\n"]
    ...
    if tool_name == "search_images" and "results" in result:
        lines = [f"🖼️ 找到 {result.get('results_count', 0)} 张图片:\n"]
        for i, r in enumerate(result['results'][:5], 1):
            if 'error' in r:
                lines.append(f"  ⚠️ {r['error']}")
            else:
                lines.append(f"  {i}. {r.get('title', '无标题')}")
                lines.append(f"     🖼️ {r.get('image_url', '')[:80]}...")
                lines.append("")
        return "\n".join(lines)
    ...
    if result.get('files'):
        file_list = "\n".join([f"  {'📁' if f['is_dir'] else '📄'} {f['name']}" for f in result['files']])
        return f"📂 共 {len(result['files'])} 个文件/目录:\n{file_list}"
...
if __name__ == '__main__':
    ...
    try:
        log(f"管理员权限: {'是' if is_admin() else '否'}")
        log(f"工作目录: {WORKSPACE}")
        log(f"Skills 目录: {SKILLS_DIR}")
        
        log(f"已加载 Skills: {list(skill_manager.skills.keys())}")
        ...
        print(f"🧩 已加载 Skills: {list(skill_manager.skills.keys())}")
        print(f"💾 上下文限制: {MAX_MESSAGES} 条消息 / {MAX_TOTAL_TOKENS} tokens")
        print("=" * 50)
        app.run(debug=False, host='0.0.0.0', port=5000, threaded=True)
        
    except Exception as e:
        import traceback
        err_msg = f"❌ 程序异常退出: {str(e)}\n{traceback.format_exc()}"
        log(err_msg)
        print(err_msg)
